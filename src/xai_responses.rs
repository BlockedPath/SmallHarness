...
const XAI_GROK_CLIENT_VERSION: &str = "0.2.16";

fn resolve_xai_url(base_url: &str) -> String {
...
fn normalized_xai_model_id(model_id: &str) -> String {
    model_id
        .to_lowercase()
        .split('/')
        .last()
...
fn is_grok_cli_proxy_model(model_id: &str) -> bool {
    let normalized = normalized_xai_model_id(model_id);
    normalized == "grok-build" || normalized == "grok-composer-2.5-fast"
}

fn resolve_xai_url_for_model(base_url: &str, model_id: &str) -> String {
    if is_grok_cli_proxy_model(model_id) {
        let base = std::env::var("XAI_CLI_BASE_URL").unwrap_or_else(|_| XAI_CLI_BASE_URL.into());
        resolve_xai_url(&base)
    } else {
        resolve_xai_url(base_url)
    }
}

fn grok_cli_session_id() -> String {
    let nanos = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    format!("small-harness-{}-{nanos}", std::process::id())
}

fn xai_model_request_headers(model_id: &str, session_id: &str) -> Vec<(&'static str, String)> {
    if !is_grok_cli_proxy_model(model_id) {
        return Vec::new();
    }
    vec![
        ("x-grok-client-identifier", "small-harness".into()),
        ("x-grok-client-version", XAI_GROK_CLIENT_VERSION.into()),
        ("x-xai-token-auth", "xai-grok-cli".into()),
        ("x-grok-model-override", normalized_xai_model_id(model_id)),
        ("x-grok-conv-id", session_id.to_string()),
    ]
}
    ...
    match content {
        ...
        UserContent::Parts(parts) => parts
            ...
            .map(|part| match part {
                ...
                UserContentPart::ImageUrl { image_url } => json!({
                    "type": "input_image",
...
fn build_request_body(req: &ChatRequest<'_>) -> Value {
    let model = canonical_xai_model(req.model).unwrap_or(req.model);
...
pub async fn stream_xai_responses<F>(
...
where
    F: FnMut(StreamChunk),
{
    let access_token = if backend.api_key.is_empty() {
        xai_oauth::access_token(client).await?
    } else {
        backend.api_key.clone()
    };
    let model = canonical_xai_model(req.model).unwrap_or(req.model);
    let url = resolve_xai_url_for_model(&backend.base_url, model);
    let body = build_request_body(req);
    let session_id = grok_cli_session_id();
    let mut builder = client
        .post(url)
        .bearer_auth(&access_token)
        .header("accept", "text/event-stream")
        .header("content-type", "application/json");
    for (name, value) in xai_model_request_headers(model, &session_id) {
    ...
    if !resp.status().is_success() {
        ...
        return Err(anyhow!("xAI HTTP {}: {}", status, body.trim()));
    }
...
pub fn xai_model_list() -> Vec<String> {
    vec![
        "grok-4.3".into(),
        "grok-build".into(),
...
/// Canonical model ids from the pi-xai-oauth provider.  Accept a few shorthands
/// users naturally type, while leaving unknown model ids untouched elsewhere so
/// direct xAI API-key users can still target future models.
pub fn canonical_xai_model(model: &str) -> Option<&'static str> {
    match model.trim() {
        "grok" | "grok-4" | "grok-4.3" | "4.3" => Some("grok-4.3"),
        "grok-build" | "build" => Some("grok-build"),
        "grok-composer-2.5-fast" | "composer" | "composer-fast" => Some("grok-composer-2.5-fast"),
        "grok-4.2" | "grok-4.20" | "grok-4.20-0309-reasoning" | "4.2" | "4.20" | "reasoning" => {
        ...
        | "fast"
        | "non-reasoning" => Some("grok-4.20-0309-non-reasoning"),
        "grok-4.20-multi-agent-0309" | "multi-agent" | "ma" => Some("grok-4.20-multi-agent-0309"),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn resolves_xai_url_variants() {
        assert_eq!(
            resolve_xai_url("https://api.x.ai/v1"),
            "https://api.x.ai/v1/responses"
        );
        assert_eq!(
            resolve_xai_url("https://api.x.ai/v1/responses"),
            "https://api.x.ai/v1/responses"
        );
        assert_eq!(
            resolve_xai_url_for_model("https://api.x.ai/v1", "grok-build"),
            "https://cli-chat-proxy.grok.com/v1/responses"
        );
    }

    #[test]
    fn proxy_models_get_grok_cli_headers() {
        let headers = xai_model_request_headers("grok-composer-2.5-fast", "session-1");
        assert!(headers
            .iter()
            .any(|(k, v)| *k == "x-grok-model-override" && v == "grok-composer-2.5-fast"));
        assert!(headers
            .iter()
    ...
    #[test]
    fn request_body_omits_tool_choice_when_no_tools_are_present() {
    ...
    #[test]
    fn request_body_omits_tool_choice_when_tools_slice_is_empty() {
        let messages = [ChatMessage::User {
            content: "hi".into(),
        }];
        let tools: [ToolDef; 0] = [];
    ...
    #[test]
    fn canonicalizes_pi_xai_oauth_models_and_aliases() {
        assert_eq!(canonical_xai_model("grok"), Some("grok-4.3"));
        assert_eq!(
            canonical_xai_model("fast"),
            Some("grok-4.20-0309-non-reasoning")
        );
        assert_eq!(canonical_xai_model("build"), Some("grok-build"));
        assert_eq!(
            canonical_xai_model("composer"),
            Some("grok-composer-2.5-fast")
        );
        assert_eq!(
            canonical_xai_model("multi-agent"),
            Some("grok-4.20-multi-agent-0309")
        );
        assert_eq!(canonical_xai_model("future-grok"), None);
    }
